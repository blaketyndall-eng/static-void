...
class CompanyOperatorWorkspaceORM(Base):
    __tablename__ = 'company_operator_workspaces'
...
class CompanyOperatorSnapshotORM(Base):
    __tablename__ = 'company_operator_snapshots'
    ...
    operating_rhythms: Mapped[list[str]] = mapped_column(JSON, default=list)
    opportunities: Mapped[list[str]] = mapped_column(JSON, default=list)
    notes: Mapped[list[dict]] = mapped_column(JSON, default=list)
    company_health_score: Mapped[float] = mapped_column(Float, default=0.0)
    execution_clarity_score: Mapped[float] = mapped_column(Float, default=0.0)
