...
from packages.services.app_blueprint_templates_bridge import specialize_build_packet_with_branches


def generate_build_packet_bridge(app_type: AppTemplateType, name: str) -> BuildPacket:
...
def build_blueprint_bridge(
    *,
    ...
    base = AppBlueprint(
        ...
        build_packet=generate_build_packet_bridge(app_type, name),
        notes=notes,
    )
    base.build_packet = specialize_build_packet_with_branches(base)
    return base


def generate_scaffold_plan_bridge(
    ...
    *,
    ...
    packet = specialize_build_packet_with_branches(blueprint)
    ...
    steps = [
        ...
        "Apply branch-aware specialization for target workflow.",
    ...
    return ScaffoldPlan(
        ...
        summary=f"Bridge-aware scaffold plan for {blueprint.name} using {blueprint.app_type.value} template.",
        steps=steps,
