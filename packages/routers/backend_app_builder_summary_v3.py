...
router = APIRouter(prefix='/api/v2/app-builder', tags=['app_builder_summary_v3'])
telemetry = TelemetryLogger(filepath='var/app_builder_telemetry.jsonl')


@router.get('/summary-v3')
def get_app_builder_summary_v3(db: Session = Depends(get_db)) -> dict:
    ...
    emit_view_event(telemetry, API_REQUEST_COMPLETED, route='get_app_builder_summary_v3', returned_count=len(blueprints), linked_count=linked_count)
    return payload
