...
router = APIRouter(prefix='/api/v2/software-engineering', tags=['software_engineering_app_production_v4'])
...
@router.get('/app-production/summary-v4')
def get_app_production_summary_v4(db: Session = Depends(get_db)) -> dict:
    blueprint_repo = AppBlueprintRepository(db)
    ...
    emit_view_event(telemetry, API_REQUEST_COMPLETED, route='get_app_production_summary_v4', returned_count=len(advisories))
    return payload


@router.get('/app-production/blueprint/{blueprint_id}/v4')
def get_blueprint_app_production_advisory_v4(blueprint_id: str, db: Session = Depends(get_db)) -> dict:
    research_repo = EngineeringResearchRepository(db)
    execution_repo = EngineeringExecutionRepository(db)
    experiment_repo = EngineeringExperimentRepository(db)
    event_repo = SystemEventRepository(db)
    ...
    if blueprint is None:
        return {'blueprint': None, 'advisory': None, 'linked_project': None, 'recent_activity': [], 'production_decision': None}
    ...
    decision = decision_repo.get_for_blueprint(blueprint_id)
    operator_actions = []
    if decision is not None:
        operator_actions.extend(decision.action_items[:3])
    elif advisory.readiness_score >= 75:
    ...
    if linked_project is not None:
        source_ids.add(linked_project.id)
    recent_activity = [item.model_dump(mode='json') for item in event_repo.list() if item.source_id in source_ids]
    payload = {
    ...
    emit_view_event(telemetry, API_REQUEST_COMPLETED, route='get_blueprint_app_production_advisory_v4', blueprint_id=blueprint_id)
    return payload
