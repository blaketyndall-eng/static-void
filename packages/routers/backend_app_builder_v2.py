...
from packages.services.app_blueprint_generator_v2 import build_blueprint_v2, generate_scaffold_plan_v2
from packages.storage.session import get_db
from telemetry_events import API_REQUEST_COMPLETED
from telemetry_helpers import emit_action_event, emit_view_event
from telemetry_logger import TelemetryLogger

router = APIRouter(prefix="/api/v1/app-builder", tags=["app_builder_v2"])
telemetry = TelemetryLogger(filepath="var/app_builder_telemetry.jsonl")


@router.post("/blueprints-v2")
def create_blueprint_v2(payload: CreateAppBlueprintRequest, db: Session = Depends(get_db)) -> dict:
    try:
    ...
    emit_action_event(telemetry, API_REQUEST_COMPLETED, route="create_blueprint_v2", blueprint_id=saved.id, app_type=saved.app_type.value)
    return saved.model_dump(mode="json")


@router.post("/blueprints-v2/{blueprint_id}/scaffold-plan")
def create_scaffold_plan_v2(blueprint_id: str, payload: GenerateScaffoldPlanRequest, db: Session = Depends(get_db)) -> dict:
    ...
    plan = generate_scaffold_plan_v2(
        blueprint,
    ...
    emit_action_event(telemetry, API_REQUEST_COMPLETED, route="create_scaffold_plan_v2", blueprint_id=blueprint_id, generated_count=len(saved.generated_files))
    return saved.model_dump(mode="json")


@router.get("/blueprints-v2/{blueprint_id}/scaffold-plan")
def get_scaffold_plan_v2(blueprint_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    emit_view_event(telemetry, API_REQUEST_COMPLETED, route="get_scaffold_plan_v2", blueprint_id=blueprint_id)
