...
router = APIRouter(prefix="/api/v1/master-console", tags=["master_console_v2"])
telemetry = TelemetryLogger(filepath="var/master_console_telemetry.jsonl")


@router.get("/summary-v2")
def get_master_console_summary_v2(db: Session = Depends(get_db)) -> dict:
    ...
    emit_view_event(
        ...
        route="get_master_console_summary_v2",
        returned_count=(len(apps) + len(blueprints) + len(marketing_projects) + len(scans) + len(investments) + len(brain_links)),
    )
    return payload
