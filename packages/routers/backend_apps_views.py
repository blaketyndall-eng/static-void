...
router = APIRouter(prefix="/api/v1/apps", tags=["apps_views"])
telemetry = TelemetryLogger(filepath="var/apps_telemetry.jsonl")


@router.get("/summary")
def get_apps_summary(db: Session = Depends(get_db)) -> dict:
    ...
    emit_view_event(telemetry, API_REQUEST_COMPLETED, route="get_apps_summary", returned_count=len(apps))
    return payload


@router.get("/{app_id}/operator-summary")
def get_app_operator_summary(app_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    if app is None:
        return {"count": 0, "app": None, "runs": [], "feedback": [], "open_feedback_count": 0}
    ...
    emit_view_event(telemetry, API_REQUEST_COMPLETED, route="get_app_operator_summary", app_id=app_id, run_count=len(runs), feedback_count=len(feedback))
    return payload
