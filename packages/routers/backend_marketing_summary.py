...
router = APIRouter(prefix="/api/v1/marketing", tags=["marketing_summary"])
telemetry = TelemetryLogger(filepath="var/marketing_telemetry.jsonl")


@router.get("/console-summary")
def get_marketing_console_summary(db: Session = Depends(get_db)) -> dict:
    ...
    emit_view_event(
        ...
        route="get_marketing_console_summary",
