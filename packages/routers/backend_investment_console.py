...
router = APIRouter(prefix="/api/v1/investment", tags=["investment_console"])
telemetry = TelemetryLogger(filepath="var/investment_telemetry.jsonl")


@router.get("/console/summary")
def get_investment_console_summary(db: Session = Depends(get_db)) -> dict:
    ...
    emit_view_event(
        ...
        route="get_investment_console_summary",
