...
def reset_state() -> None:
    ...
    if db_path.exists():
        db_path.unlink()
    for path in [
        Path('var/executive_dashboard_telemetry.jsonl'),
...
def test_executive_dashboard_summary_v2() -> None:
    reset_state()
    client = TestClient(app)
    seed_workspaces(client)

    summary = client.get('/api/v1/executive-dashboard/summary-v2')
    assert summary.status_code == 200
    payload = summary.json()
    assert 'executive_overview' in payload
    assert 'core_cards' in payload
    assert 'expansion_card' in payload
    assert payload['executive_overview']['expansion_workspace_count'] >= 8
    assert len(payload['expansion_card']['arm_summaries']) >= 8
