...
def reset_state() -> None:
    ...
    if db_path.exists():
        db_path.unlink()
    for path in [Path('var/expansion_control_telemetry.jsonl')]:
        if path.exists():
            path.unlink()
...
def test_expansion_rollup_summary() -> None:
    reset_state()
    ...
    summary = client.get('/api/v1/expansion-rollup/summary')
    assert summary.status_code == 200
    payload = summary.json()
    assert payload['total_workspace_count'] >= 8
    assert len(payload['arm_summaries']) >= 8
