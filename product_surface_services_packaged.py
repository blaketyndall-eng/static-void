...
def build_attention_needed(db) -> dict:
    ...
    for evaluation in evaluations:
        ...
        if not linked:
            items.append({"evaluation_id": evaluation_id, "title": evaluation["title"], "reason": "No recommendation exists yet.", "priority": "high"})
            continue
        proposed = [item for item in linked if item.get("status") == "proposed"]
        if not proposed:
            items.append({"evaluation_id": evaluation_id, "title": evaluation["title"], "reason": "Recommendation exists but none are proposed yet.", "priority": "medium"})
...
def build_evaluation_summary(db, evaluation_id: str) -> dict | None:
    ...
    return {"evaluation": evaluation.model_dump(mode="json"), "counts": {"criteria": len(evaluation.criteria), "evidence": len(evidence), "artifacts": len(artifacts), "recommendations": len(recommendations)}, "latest_recommendation": latest_recommendation, "artifacts": artifacts, "evidence": evidence}
...
def build_recommendation_cards(db, evaluation_id: str) -> dict | None:
    ...
    for recommendation in recommendations:
        ...
        cards.append({"id": recommendation["id"], "title": recommendation["title"], "summary": recommendation["summary"], "status": recommendation["status"], "why": recommendation.get("rationale", ""), "artifact_count": len(recommendation.get("artifact_ids", [])), "evidence_count": len(recommendation.get("evidence_ids", [])), "artifacts": linked_artifacts})
...
def build_ranked_recommendation_summary(db, evaluation_id: str) -> dict | None:
    ...
    return {"evaluation_id": evaluation_id, "count": len(ranked), "top_recommendation": top_recommendation, "recommendations": ranked}
...
def build_evaluation_readiness(db, evaluation_id: str) -> dict | None:
    ...
    score = min(min(criteria_count, 4) * 20 + min(evidence_count, 3) * 15 + min(artifact_count, 2) * 10 + (20 if recommendation_count > 0 else 0), 100)
    level = "ready" if score >= 80 else "developing" if score >= 50 else "early"
    blockers = []
    if criteria_count == 0:
    ...
    return {"evaluation_id": evaluation_id, "score": score, "level": level, "counts": {"criteria": criteria_count, "evidence": evidence_count, "artifacts": artifact_count, "recommendations": recommendation_count}, "blockers": blockers}
