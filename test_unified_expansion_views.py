...
import db_session
from app_sys_unified_expansion import app
...
def reset_state() -> None:
    ...
    for path in [
        ...
        Path('var/global_summary_telemetry.jsonl'),
...
def test_unified_expansion_master_console_v5_and_global_summary_v4() -> None:
    reset_state()
    client = TestClient(app)
    seed_workspaces(client)

    master = client.get('/api/v1/master-console/summary-v5')
    assert master.status_code == 200
    master_payload = master.json()
    assert 'expansion' in master_payload
    assert master_payload['expansion']['total_workspace_count'] >= 8
    assert len(master_payload['expansion']['arm_summaries']) >= 8

    global_summary = client.get('/api/v1/global/summary-v4')
    assert global_summary.status_code == 200
    global_payload = global_summary.json()
    assert 'expansion' in global_payload
    assert global_payload['expansion_workspace_count'] >= 8
    assert len(global_payload['expansion']['arm_summaries']) >= 8
